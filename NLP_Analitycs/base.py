...
def read_data(path: str, columns: list):
    ...
    """
    fuentes = list(pd.Series(os.listdir(path))[
                   pd.Series(os.listdir(path)).str.contains('.csv')])
    df = pd.DataFrame(columns=columns)

    for fuente in fuentes:
        if (fuente == "larepublica.csv"):
            df_ = pd.read_csv(path+fuente, sep=',', encoding='utf-8-sig')
            df_['Fuente'] = fuente
        else:
            df_ = pd.read_csv(path+fuente, sep=',', encoding='utf-8-sig')
            df_['Fuente'] = fuente
...
def pre_proc_text(df_: pd.DataFrame, tipo: str):
    ...
    texts = texts.apply(
        lambda x: ' '.join(pd.Series(re.split(' ', x))[
                           ~pd.Series(re.split(' ', x)).isin(stop_words)].tolist())
    )
...
def useLDA(texts: list, n: int):
    ...
    lda_model = LdaMulticore(bow_corpus, num_topics=n,
                             id2word=dictionary, passes=2, workers=2)
    # show_topics()[0][1]
    return lda_model.show_topics(num_topics=1, num_words=5, log=False, formatted=False)
